...
class MovedMemberForm(BaseMembershipForm):
    ...
    def clean(self):
        ...
        if cleaned_data.get('moved_household') == NO and cleaned_data.get('moved_community') == YES:
            raise forms.ValidationError(
                'You have indicated that the participant has NOT moved out of the household but HAS '
        ...
        if cleaned_data.get('moved_community') != YES and cleaned_data.get('new_community'):
            raise forms.ValidationError(
                'Participant has NOT moved out of the community. Do not specify a new community.')
